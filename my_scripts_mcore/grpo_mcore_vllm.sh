...
project_name='DAPO-Qwen1.5-moe'
exp_name='DAPO-Qwen1.5-moe-megatron-vllm'
...
# Paths
MODEL_PATH=/root/hf_cache/hub/models/Qwen/Qwen3-30B-A3B

RAY_DATA_HOME=${RAY_DATA_HOME:-"/root/data"}
TRAIN_FILE=$RAY_DATA_HOME/dapo_data_math/dapo-math-17k.parquet
TEST_FILE=$RAY_DATA_HOME/dapo_data_math/aime-2024.parquet

# quick guards (will abort if files are missing)
[ -s "$TRAIN_FILE" ] || { echo "Missing $TRAIN_FILE"; exit 1; }
[ -s "$TEST_FILE" ]  || { echo "Missing $TEST_FILE";  exit 1; }
...
USE_MBRIDGE=True
USE_DIST_CKPT=True
DIST_CKPT_PATH=/root/mcore_models/Qwen3-30B-A3B

SAVE_DIR=${SAVE_DIR:-/data/verl_ckpt/moe/${exp_name}}
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_megatron_trainer'\
    ...
    +actor_rollout_ref.ref.checkpoint.async_save=true \
