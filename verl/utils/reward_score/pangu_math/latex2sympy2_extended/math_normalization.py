...
class NormalizationConfig:
    ...
    nits: bool = False
    boxed: Literal["all", "last", "none"] = "none"
...
tight_units_regex = re.compile("|".join([f"\\s?(?:{unit})($|\\W)" for unit in tight_units]))
...
def _fix_malformed_operators(text: str) -> str:
    ...
    for symbol, latex_symbol in SYMBOL_UNIFY.items():
        expr_str = expr_str.replace(symbol, latex_symbol)
    for token, latex_expr in SPECIAL_TOKEN_UNIFY.items():
        expr_str = expr_str.replace(token, latex_expr)
    return expr_str
...
def extract_boxed_content(text: str, mode: Literal["last", "all", "none"] = "last") -> str:
    """
    ...
    def find_content_boundaries(text: str, opening_brace_pos: int, max_pos: int) -> Union[tuple[int, int], None]:
...
def _fix_operator(text: str) -> str:
    for operator in OPER_SYMB_TO_FIX:
        pa = rf"(?<!\\){operator}"
        text = re.sub(pa, rf"\\{operator}", text)
    return text
...
def normalize_latex(text: str, config: NormalizationConfig) -> str:
    ...
    if config.basic_latex:
        ...
        text = text.replace(r'\mathrm{e}', 'e')
    ...
    if config.units:
        ...
        for _ in range(2):
            ...
            if _text != "" and _text != text:
                text = _text
        for _ in range(2):
            _text = tight_units_regex.sub(r"\1\2", text)
            if _text != "" and _text != text:
                text = _text
